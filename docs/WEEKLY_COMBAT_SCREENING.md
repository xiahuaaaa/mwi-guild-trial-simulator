...
水/火合计不够两边各 2 个烟爆/冰霜座位时（例如 3 人只能 1/2）：缺的那一侧用最弱自然分开补，一人烟爆、一人冰霜爆裂。刺猬补位包是 **群疗 / 增幅 / 烟爆或冰霜 / 缠绕**；虫群是 **群疗 / 菌幕 / 烟爆或冰霜 / 缠绕**。刺猬粉尘仍至少 2（能留 3 就留 3），不占用这两名补位自然。转输出时跳过补位奶。
...
- 超人数一侧溢到未满一侧（`applyTeamCaps`）。獾周未满的獾侧按 **枪→剑→弓→弩→锤** 从虫群物理溢出里抽人；火/水先占獾座位。刺猬周未满的刺猬侧按 **火→弩→剑→弓→自→锤** 填（优先溢出弩，装不下的溢出自也去刺猬；不抽水）。
